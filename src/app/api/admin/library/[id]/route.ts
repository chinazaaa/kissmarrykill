...
export async function PATCH(
  ...
  if (action === 'approve') {
    ...
    // Full field edit
    if (title !== undefined) {
      if (typeof title !== 'string' || title.trim().length === 0) return NextResponse.json({ error: 'Invalid title' }, { status: 400 })
      if (title.trim().length > 100) return NextResponse.json({ error: 'Title too long' }, { status: 400 })
      updates.title = title.trim()
    }
    if (game_type !== undefined) {
      if (!VALID_GAME_TYPES.includes(game_type)) return NextResponse.json({ error: 'Invalid game_type' }, { status: 400 })
      updates.game_type = game_type
    }
    if (author_name !== undefined) {
      if (typeof author_name !== 'string' || author_name.trim().length === 0) return NextResponse.json({ error: 'Invalid author_name' }, { status: 400 })
    ...
    if (description !== undefined) {
      if (description !== null && typeof description === 'string' && description.length > 500) return NextResponse.json({ error: 'Description too long' }, { status: 400 })
