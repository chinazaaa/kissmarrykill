...
export async function POST(req: NextRequest, { params }: { params: Promise<{ code: string }> }) {
  ...
  const limitKey = lobbyType as LobbyLimitGameType
  ...
  if (timer_seconds !== undefined) {
    gameUpdate.timer_seconds = clampBoardGameTurnTimer(timer_seconds, lobbyType)
  }

  if (game_duration_seconds !== undefined) {
    if (lobbyType === 'monopoly') {
      gameUpdate.game_duration_seconds = clampMonopolyGameDuration(game_duration_seconds)
    } else if (lobbyType === 'whot') {
