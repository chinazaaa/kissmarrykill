...
export function TicTacToeShell({
  children,
  title,
  subtitle,
  wide,
          ...
          {(title || subtitle) && !compact && (
            <header className="text-center space-y-2">
              <div className="inline-flex items-center justify-center gap-2">
                ...
                <GameTypeBadge gameType="tic_tac_toe" />
              </div>
              {title && (
...
export function TicTacToeCard({ children, className = '' }: { children: ReactNode; className?: string }) {
  return <div className={`glass-card ${className}`}>{children}</div>
}

export function TicTacToePrimaryButton({
  children,
  onClick,
  disabled,
  loading,
...
export function TicTacToeSecondaryButton({
  children,
  onClick,
  disabled,
  className = '',
...
export function TicTacToeLoadingScreen() {
  return (
    <TicTacToeShell>
      <TicTacToeCard className="p-8 text-center">
...
export function TicTacToeTurnBar({
  turnPlayerName,
  isMyTurn,
  secondsLeft,
  hasTimer,
