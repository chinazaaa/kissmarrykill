          ...
          {(title || subtitle) && !compact && (
            <header className="text-center space-y-2">
              <div className="inline-flex items-center justify-center gap-2">
                ...
                <GameTypeBadge gameType="checkers" />
              </div>
              {title && (
...
export function CheckersCard({ children, className = '' }: { children: ReactNode; className?: string }) {
  return <div className={`glass-card ${className}`}>{children}</div>
}

export function CheckersPrimaryButton({
  children,
  onClick,
  disabled,
  loading,
...
export function CheckersSecondaryButton({
  children,
  onClick,
  disabled,
  className = '',
...
export function CheckersLoadingScreen() {
  return (
    <CheckersShell>
      <CheckersCard className="p-8 text-center">
...
export function CheckersTurnBar({
  turnPlayerName,
  isMyTurn,
  mustContinue,
}: {
