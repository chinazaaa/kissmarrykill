...
export async function fetchMltQuestionUsage(
...
export async function fetchWyrQuestionUsage(
  supabase: SupabaseClient
): Promise<Map<string, number>> {
  const { data } = await supabase
    .from('rounds')
    .select('wyr_option_a, wyr_option_b')
    .not('wyr_option_a', 'is', null)

  return countValues(
    (data ?? []).map((row) =>
      row.wyr_option_a && row.wyr_option_b
        ? wyrQuestionKey(row.wyr_option_a, row.wyr_option_b)
        : null
    )
  )
}
