...
import { processCrazyEightsExpireTurn } from './crazy-eights'
...
// fast/throttled tab or a dropped /expire-crazy-eights request lets turns keep advancing
// past time while the display reads 0:00). These tests pin that the turn-expiry path,
// which every viewing client reliably pokes, finalizes the game by lowest hand once the
// buzzer has passed — and leaves an in-time game alone.
    ...
    from(table: string) {
      return {
        select() {
          ...
          if (table === 'crazy_eights_player_hands') return chain({ data: opts.hands, error: null })
        ...
        update(vals: Record<string, unknown>) {
          ...
          // finishByLowestHand's CAS update ends with .select('game_id') and checks for a
          // non-empty data array; return one so it counts as "won the race".
...
function baseInputs(sessionStartedAt: string, durationSeconds: number) {
  return {
    session: {
      ...
      direction: 1,
      finish_order: [],
      top_card: { id: 'hearts-3', suit: 'hearts', rank: 3 },
    ...
    // A holds a low total (3), B a high total (K+Q = 10+10 = 20). Lowest-hand winner is A.
    hands: [
      { player_id: 'A', cards: [{ id: 'hearts-3', suit: 'hearts', rank: 3 }], player_order: 0 },
      {
        player_id: 'B',
        cards: [
          { id: 'spades-13', suit: 'spades', rank: 13 },
          { id: 'clubs-12', suit: 'clubs', rank: 12 },
        ],
        player_order: 1,
      },
    ],
...
describe('processCrazyEightsExpireTurn — game-clock enforcement', () => {
  it('finalizes by lowest hand once the game duration has elapsed', async () => {
    ...
    const result = await processCrazyEightsExpireTurn(m.supabase, 'GAME1')

    expect(result.error).toBeUndefined()
    const finish = m.updates.find((u) => u.table === 'crazy_eights_sessions' && u.vals.phase === 'finished')
  ...
  it('leaves an in-time game running (no finish write)', async () => {
    ...
    expect(m.updates.some((u) => u.table === 'crazy_eights_sessions' && u.vals.phase === 'finished')).toBe(false)
  })
