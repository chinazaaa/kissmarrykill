 ...
 * their own isolated <CheckersClockChip> components, keeping moves smooth and the
 ...
 */
export function useCheckersClockExpiry(gameCode: string, session: CheckersSession | null, enabled: boolean) {
  ...
  useEffect(() => {
    ...
    const id = window.setInterval(() => {
      ...
      if (remaining <= 0 && !expiringRef.current) {
        ...
        void fetch('/api/checkers/expire-turn', {
          method: 'POST',
